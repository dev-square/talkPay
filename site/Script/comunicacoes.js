...
function conversation() {
    for(i = 1; i <= intentions.length-1;i++){
        ...
        {
            //EFETIVAÇÃO DA ETAPA DE PROCESSAMENTO DA INTENT
            if (intentions[i].process != "") intentions[i].doProcess();
...
class intent{
    ...
    doProcess() {
        switch(this.process){
            case 'hideHome':
                document.getElementById('card-visitors').className = "hidden";
                break;
        }
    }
}
function intentMk(context,tp,response,nextContext = 0,field="",process=""){
    Intent = new intent();
...
intentMk(3,'Mercado Pago','Informe o e-mail da conta para a qual enviar o dinheiro',4,'txtEmail','hideHome');
